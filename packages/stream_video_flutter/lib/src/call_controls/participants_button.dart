...
class StreamParticipantsButton extends StatelessWidget {
  /// Creates a button over the people in [call].
  ///
  /// The count follows the call's participants as they come and go. Use
  /// [StreamParticipantsButton.forParticipants] where the people are not a
  /// joined call's — in a lobby, which lists who is already inside.
  const StreamParticipantsButton({super.key, required Call call, this.onTap})
    : _call = call,
      _participants = null;

  /// Creates a button over [participants], a list the caller keeps.
  const StreamParticipantsButton.forParticipants({
    super.key,
