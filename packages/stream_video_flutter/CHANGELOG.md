...
- `StreamConnectionQualityIndicatorStyle` takes a `decoration` in place of `backgroundColor`, so the chip can be reshaped and not only recolored, and a `strokeWidth` for the thickness of its bars.
