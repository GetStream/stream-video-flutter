...
- `StreamMicrophoneButton` and `StreamCameraButton` take an optional `devices`, and badge a device the platform does not report.
- Added `CallControlBar`, which lays a call's controls out per screen size from a `CallControlBarLayout` of `leading`, `center` and `trailing` slots, falling back to the next smaller layout that was given.
...
- Added an overflow menu to the participant tile. Supply `actions`, or `actionsBuilder` for a menu that depends on the participant, and the SDK renders them; supply nothing and the button is not drawn at all. Passing both is an error, and a fixed list reaches `StreamParticipantTileProps` as a builder, so a replacement component has one field to substitute. `StreamParticipantTileAction` is an icon, a label and a callback, so what an action does — pinning, muting, blocking, removing — stays with the integrator:
...
- The name pill still reports a participant whose inbound video the SFU has paused, which `StreamParticipantLabelStyle.videoPausedColor` and `videoPausedIconSize` style.
...
- The Android picture-in-picture window shows the video alone: no name pill, connection quality indicator or overflow button.
...
- `StreamLobbyView` reads the call it is a waiting room for and no longer creates one that is not there. Create the call before showing the lobby, or from `onJoinCallPressed` — which is what a host that decides something at creation time, an encryption mode above all, has to do. A call that does not exist yet leaves `StreamLobbyController.fetchError` set and its participant list empty.
- `StreamLobbyView` builds no `Scaffold` or `AppBar` of its own, so it can be embedded in a screen that already has one — wrap it in whatever chrome the app needs. `onCloseTap` is therefore gone, along with `backgroundColor`, `cardBackgroundColor`, `userAvatarTheme` and `participantAvatarTheme`; appearance comes from the theme, and extra buttons from `StreamLobbyActions`. It gains `actions`, `title`, `subtitle`, `joinButtonLabel`, `footer`, `joinEnabled` and `controller`.
- `StreamLobbyView.onJoinCallPressed` is a `StreamLobbyJoinCallback`: it may be async and returns whether the join happened. Return true once the call has been joined, or once whoever joins it has the options; return false and the lobby takes its tracks back and carries on previewing, so a call that could not be created leaves the user where they were. An existing callback needs a `return true`.
- The lobby's card listing who is already in the call is gone, and `StreamLobbyParticipantsView` with it. `StreamLobbyParticipantsButton` replaces it: a badge with the number waiting, opening the list on tap.
