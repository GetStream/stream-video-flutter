...
- Added `filterQuality` to `StreamVideoRenderer`, `VideoTrackRenderer`, `StreamCallParticipant` and `StreamCallParticipantThemeData`, so the sampling quality used to scale video tiles can be overridden per widget or per theme. The default is exposed as `defaultVideoFilterQuality`.
