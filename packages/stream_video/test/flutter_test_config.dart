...
/// Registers a `stream_webrtc_flutter`-backed [StreamWebRtc] before any test
/// runs, mirroring `stream_video_flutter`'s `FlutterWebRtcEngine` (which this
/// package's tests can't depend on directly — that would be a package
/// cycle). Without this, code paths that touch [StreamWebRtc.instance]
/// (e.g. [RtcMediaDeviceNotifier]) hit the unregistered default and throw.
Future<void> testExecutable(FutureOr<void> Function() testMain) async {
  StreamWebRtc.instance = _TestStreamWebRtc();
  await testMain();
}

class _TestStreamWebRtc implements StreamWebRtc {
  @override
  ...
  Future<StreamNativeFactory?> createNativeFactory({
    ...
    return _TestNativeFactory(factory);
  }

  @override
  StreamWebRtcHelper get helper => const _TestStreamWebRtcHelper();
...
class _TestNativeFactory implements StreamNativeFactory {
  _TestNativeFactory(this._factory);
...
class _TestStreamWebRtcHelper implements StreamWebRtcHelper {
  const _TestStreamWebRtcHelper();
