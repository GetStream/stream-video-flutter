    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'date'),
            'Required key "DailyAggregateCallDurationReportResponse[date]" is missing from JSON.');
        assert(json[r'date'] != null,
            'Required key "DailyAggregateCallDurationReportResponse[date]" has a null value in JSON.');
        assert(json.containsKey(r'report'),
            'Required key "DailyAggregateCallDurationReportResponse[report]" is missing from JSON.');
        assert(json[r'report'] != null,
            'Required key "DailyAggregateCallDurationReportResponse[report]" has a null value in JSON.');
        return true;
      }());
