//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class ParticipantSeriesUserStats {
  /// Returns a new [ParticipantSeriesUserStats] instance.
  ParticipantSeriesUserStats({
    this.metrics = const {},
    this.metricsMeta = const {},
    this.metricsOrder = const [],
    this.thresholds = const {},
  });

  Map<String, List<List<double>>> metrics;

  Map<String, MetricDescriptor> metricsMeta;

  List<String> metricsOrder;

  Map<String, List<MetricThreshold>> thresholds;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ParticipantSeriesUserStats &&
          _deepEquality.equals(other.metrics, metrics) &&
          _deepEquality.equals(other.metricsMeta, metricsMeta) &&
          _deepEquality.equals(other.metricsOrder, metricsOrder) &&
          _deepEquality.equals(other.thresholds, thresholds);

  @override
  int get hashCode =>
      // ignore: unnecessary_parenthesis
      (metrics.hashCode) +
      (metricsMeta.hashCode) +
      (metricsOrder.hashCode) +
      (thresholds.hashCode);

  @override
  String toString() =>
      'ParticipantSeriesUserStats[metrics=$metrics, metricsMeta=$metricsMeta, metricsOrder=$metricsOrder, thresholds=$thresholds]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    json[r'metrics'] = this.metrics;
    json[r'metrics_meta'] = this.metricsMeta;
    json[r'metrics_order'] = this.metricsOrder;
    json[r'thresholds'] = this.thresholds;
    return json;
  }

  /// Returns a new [ParticipantSeriesUserStats] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ParticipantSeriesUserStats? fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();

      // Ensure that the map contains the required keys.
      // Note 1: the values aren't checked for validity beyond being non-null.
      // Note 2: this code is stripped in release mode!
      assert(() {
        return true;
      }());

      return ParticipantSeriesUserStats(
        // MANUAL_EDIT: _mapListListDoubleFromJson instead of List.mapListFromJson
        metrics: json[r'metrics'] == null
            ? const {}
            : _mapListListDoubleFromJson(json[r'metrics']),
        metricsMeta: MetricDescriptor.mapFromJson(json[r'metrics_meta']),
        metricsOrder: json[r'metrics_order'] is Iterable
            ? (json[r'metrics_order'] as Iterable)
                .cast<String>()
                .toList(growable: false)
            : const [],
        thresholds: json[r'thresholds'] == null
            ? const {}
            : MetricThreshold.mapListFromJson(json[r'thresholds']),
      );
    }
    return null;
  }

  static List<ParticipantSeriesUserStats> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <ParticipantSeriesUserStats>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ParticipantSeriesUserStats.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
    }
    return result.toList(growable: growable);
  }

  static Map<String, ParticipantSeriesUserStats> mapFromJson(dynamic json) {
    final map = <String, ParticipantSeriesUserStats>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ParticipantSeriesUserStats.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
    }
    return map;
  }

  // maps a json object with a list of ParticipantSeriesUserStats-objects as value to a dart map
  static Map<String, List<ParticipantSeriesUserStats>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<ParticipantSeriesUserStats>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ParticipantSeriesUserStats.listFromJson(
          entry.value,
          growable: growable,
        );
      }
    }
    return map;
  }

  /// The list of required keys that must be present in a JSON.
  static const requiredKeys = <String>{};
}
