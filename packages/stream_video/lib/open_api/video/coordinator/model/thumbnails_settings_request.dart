...
class ThumbnailsSettingsRequest {
  ...
  bool operator ==(Object other) =>
      ...
      other is ThumbnailsSettingsRequest && other.enabled == enabled;
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "ThumbnailsSettingsRequest[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "ThumbnailsSettingsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<ThumbnailsSettingsRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = ThumbnailsSettingsRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
