...
class UnblockUserRequest {
  ...
  bool operator ==(Object other) =>
      ...
      other is UnblockUserRequest && other.userId == userId;
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'user_id'),
            'Required key "UnblockUserRequest[user_id]" is missing from JSON.');
        assert(json[r'user_id'] != null,
            'Required key "UnblockUserRequest[user_id]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<UnblockUserRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = UnblockUserRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
