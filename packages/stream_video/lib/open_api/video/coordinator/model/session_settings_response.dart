...
class SessionSettingsResponse {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SessionSettingsResponse &&
  ...
  @override
  String toString() => 'SessionSettingsResponse[inactivityTimeoutSeconds=$inactivityTimeoutSeconds]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SessionSettingsResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SessionSettingsResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SessionSettingsResponse(
        inactivityTimeoutSeconds: mapValueOfType<int>(json, r'inactivity_timeout_seconds')!,
      );
    }
    return null;
  }

  static List<SessionSettingsResponse> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<SessionSettingsResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SessionSettingsResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SessionSettingsResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
