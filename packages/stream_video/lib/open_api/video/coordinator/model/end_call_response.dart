...
class EndCallResponse {
  ...
  bool operator ==(Object other) =>
      ...
      other is EndCallResponse && other.duration == duration;
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'duration'),
            'Required key "EndCallResponse[duration]" is missing from JSON.');
        assert(json[r'duration'] != null,
            'Required key "EndCallResponse[duration]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<EndCallResponse>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = EndCallResponse.listFromJson(
          entry.value,
          growable: growable,
        );
      }
