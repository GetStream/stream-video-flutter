    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'call_cid'),
            'Required key "CallHLSBroadcastingFailedEvent[call_cid]" is missing from JSON.');
        assert(json[r'call_cid'] != null,
            'Required key "CallHLSBroadcastingFailedEvent[call_cid]" has a null value in JSON.');
        assert(json.containsKey(r'created_at'),
            'Required key "CallHLSBroadcastingFailedEvent[created_at]" is missing from JSON.');
        assert(json[r'created_at'] != null,
            'Required key "CallHLSBroadcastingFailedEvent[created_at]" has a null value in JSON.');
        assert(json.containsKey(r'type'),
            'Required key "CallHLSBroadcastingFailedEvent[type]" is missing from JSON.');
        assert(json[r'type'] != null,
            'Required key "CallHLSBroadcastingFailedEvent[type]" has a null value in JSON.');
        return true;
      }());
