...
class BlockUserRequest {
  ...
  bool operator ==(Object other) =>
      ...
      other is BlockUserRequest && other.userId == userId;
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'user_id'),
            'Required key "BlockUserRequest[user_id]" is missing from JSON.');
        assert(json[r'user_id'] != null,
            'Required key "BlockUserRequest[user_id]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<BlockUserRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BlockUserRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
