...
class CallsPerDayReportResponse {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CallsPerDayReportResponse &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CallsPerDayReportResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CallsPerDayReportResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CallsPerDayReportResponse(
        daily: DailyAggregateCallsPerDayReportResponse.listFromJson(json[r'daily']),
      );
    }
    return null;
  }

  static List<CallsPerDayReportResponse> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<CallsPerDayReportResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CallsPerDayReportResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CallsPerDayReportResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
