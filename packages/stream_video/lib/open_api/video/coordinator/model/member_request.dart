...
class MemberRequest {
  ...
  @override
  String toString() =>
      'MemberRequest[custom=$custom, role=$role, userId=$userId]';
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'user_id'),
            'Required key "MemberRequest[user_id]" is missing from JSON.');
        assert(json[r'user_id'] != null,
            'Required key "MemberRequest[user_id]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<MemberRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = MemberRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
