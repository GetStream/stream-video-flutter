...
class GetCallTypeResponse {
  ...
  @override
  String toString() => 'GetCallTypeResponse[createdAt=$createdAt, duration=$duration, grants=$grants, name=$name, settings=$settings, updatedAt=$updatedAt]';
  ...
  static Map<String, List<GetCallTypeResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GetCallTypeResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
