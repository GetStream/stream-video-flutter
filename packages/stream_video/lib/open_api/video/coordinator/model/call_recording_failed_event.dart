    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'call_cid'),
            'Required key "CallRecordingFailedEvent[call_cid]" is missing from JSON.');
        assert(json[r'call_cid'] != null,
            'Required key "CallRecordingFailedEvent[call_cid]" has a null value in JSON.');
        assert(json.containsKey(r'created_at'),
            'Required key "CallRecordingFailedEvent[created_at]" is missing from JSON.');
        assert(json[r'created_at'] != null,
            'Required key "CallRecordingFailedEvent[created_at]" has a null value in JSON.');
        assert(json.containsKey(r'egress_id'),
            'Required key "CallRecordingFailedEvent[egress_id]" is missing from JSON.');
        assert(json[r'egress_id'] != null,
            'Required key "CallRecordingFailedEvent[egress_id]" has a null value in JSON.');
        assert(json.containsKey(r'recording_type'),
            'Required key "CallRecordingFailedEvent[recording_type]" is missing from JSON.');
        assert(json[r'recording_type'] != null,
            'Required key "CallRecordingFailedEvent[recording_type]" has a null value in JSON.');
        assert(json.containsKey(r'type'),
            'Required key "CallRecordingFailedEvent[type]" is missing from JSON.');
        assert(json[r'type'] != null,
            'Required key "CallRecordingFailedEvent[type]" has a null value in JSON.');
        return true;
      }());
