...
import '../../open_api/video/coordinator/api.dart' show ApiException;
...
class ClientEventErrorMapper {
  ...
  ClientEventFailure map(Object? error) {
    ...
    // An ApiException means the server responded (with a status) — not offline.
    if (cause is ApiException) {
      return ClientEventFailure(
        ClientEventStandardCode.serverError,
        cause.message ?? 'API error ${cause.code}',
      );
    }
  ...
  bool _looksOffline(Object cause) {
    final raw = cause is http.ClientException
        ? cause.message
        : cause.toString();
