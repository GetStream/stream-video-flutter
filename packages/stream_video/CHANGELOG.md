...
- `CallPreferences` gained an `adaptiveCaptureEnabled` getter. `DefaultCallPreferences` supplies it (defaulting to `true`); a custom `CallPreferences` implementation has to add it.
- `PeerConnectionStatsBundle` is now built from a single `RtcStatsSnapshot` (`snapshot:`) instead of separate `stats`/`printable`/`raw` arguments. Reading a bundle is unchanged — `raw`, `stats` and `printable` are all still there, the latter two as lazily derived getters — but its equality and `hashCode` now only consider `peerType` and `raw`, since the other two are pure functions of it.
...
- Stats collection no longer runs when nothing observes the result. `StatsReporter` skips the whole tick — two `getStats()` round trips, each walking every report — unless something is subscribed to `Call.stats`, something is listening to the reporter itself (via `addListener` or its `stream`), or first-frame telemetry still needs a sample. Setting `CallPreferences.callStatsReportingInterval` to `Duration.zero` now disables the reporter outright.
...
- Bumped `stream_webrtc_flutter` to 3.0.2, which adds the report-type-filtered `getStats` call, the camera capture-format reconfiguration and the iOS thermal-pressure observer that the changes above build on.
...
- Fixed the publish-option fallbacks being dead code, which left the SDK silently dependent on the SFU populating every optional field. `fps`, `bitrate`, `maxSpatialLayers` and `maxTemporalLayers` arrive as proto3 scalars, so an omitted field decoded as `0` rather than `null` and every `?? default` downstream was unreachable. An SFU that omitted `maxSpatialLayers` would have published **zero** encodings, and one that omitted `fps` would have set `maxFramerate: 0` on every layer. Absent fields now map to `null`, and an explicit `0` falls back to the documented default (30 fps, 3 spatial/temporal layers, preset bitrate) instead of being published verbatim. An omitted `videoDimension` and an omitted audio `bitrate` are mapped to `null` the same way, a `maxSpatialLayers` above the three available rids is clamped instead of overflowing the rid list, and `SfuPublishOptions.toDTO()` leaves `videoDimension` absent rather than sending an explicit 0x0.
...
- Fixed a potential iOS crash on call end when disabling noise cancellation.
