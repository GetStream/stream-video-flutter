...
One of the primary low-level widgets we offer is `StreamVideoRenderer`. As the name suggests, this widget is specifically designed to render the video track of a call participant.
It also exposes callbacks that can be utilized to handle sizing changes, placeholder content, and other related functionalities.

However, since `StreamVideoRenderer` is relatively basic, we have also introduced `StreamCallParticipant` as an extended version.
It adds several extra features on top of `StreamVideoRenderer`, such as connection quality indicators, microphone indicators, and more.
