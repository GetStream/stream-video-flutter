...
If you want to customize it, providing different preset or even your own custom sorting coparator, you can do it by providing `sort` parameter to `StreamCallParticipants` widget and using it in the `StreamCallContent's` `callParticipantsBuilder`:
