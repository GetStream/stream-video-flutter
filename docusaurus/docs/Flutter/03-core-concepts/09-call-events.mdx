...
There are multiple events that you can listen to during a call. You can use them to update the UI, show notifications, or log the call events. You can listen to them vis `callEvents` stream in the `Call` object.
...
We also support custom events. This is basically just a realtime layer that you can broadcast your own events to.

### Sending custom events

You can use the `sendCustomEvent` method of the `Cal`l instance to send custom events:
