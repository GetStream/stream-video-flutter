...
- `CallState` properties that contain some of the webRTC statistics processed and more usefull for display purposes

## Stats stream

If you want to tap in directly into the stream of webRTC stats you can do this with `stats` steam inside `Call` object. It provides a stream of `CallStats` objects for publisher and subscriber connections.
We provide those statistics in three ways for ease of use:

* `raw` - raw stats as they come from the webRTC
* `printable` - representation of the stats that can be easly printed to the console or as a block of text
...
You can check the example of the stats screen in our [dogfooding app](https://github.com/GetStream/stream-video-flutter/blob/main/dogfooding/lib/screens/call_stats_screen.dart)
