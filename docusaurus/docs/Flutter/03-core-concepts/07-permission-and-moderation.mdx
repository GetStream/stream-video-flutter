...
Permissions for a video call can be complex, particularly for use cases like educational apps, live events, audio rooms, and livestreams. These use cases require detailed permissions to ensure proper functionality and user privacy.

### Current Permissions

To check if a user has a certain permission, such as transmitting audio, video, or screensharing, you can use the `hasPermission` method on the `Call` class:
...
As with all calls, there may be times where the permissions granted to a user may need to be revoked or the user banned, placed on mute, or some other action taken to limit user interaction.

To facilitate these requests, the SDK provides several methods for limiting user interaction during the call lifecycle.

**Revoke Permissions**

Similar to its sister method `grantPermissions`, `revokePermissions` lives on the current `Call` object and allows users to easily remove permissions currently assigned to a user by passing in the user id and the list of permissions to be revoked.
...
In the above example, we are only muting a single user. However, `muteUsers` does allow us to mute multiple users since it accepts a list of userIDs.
