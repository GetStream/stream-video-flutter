...
Developers can supply a list of custom options for their call using the default constructor. It's common to use a combination of both Stream default options and custom options. For example, you can use Stream's leave call, microphone, and camera buttons, but include a custom option to perform an activity unique to your application, such as sending a custom event or reaction.
...
As an example, the above snippet demonstrates how a custom `CallControlOption` can display a chat dialog while a user is on a call.