...
One of the primary low-level widgets we offer is `StreamVideoRenderer`. As the name suggests, this widget is solely responsible for rendering the video track of a call participant and exposing callbacks that can be used to respond to sizing changes, placeholder content, and more.

However, since `StreamVideoRenderer` is relatively basic, we also included `StreamCallParticipant`, which adds several extra features on top of `StreamVideoRenderer`, such as connection quality indicators, microphone indicators, and more.
...
Video Render is the lowest level for displaying a participant's video in Flutter. Unlike `StreamCallParticipant`, the option exposed by `StreamVideoRenderer` is minimal and focuses more on video fit and layout, which track types to display, and callbacks for handling changes to the video rendering.

Both `StreamVideoRenderer` and `StreamCallParticipant` can be used as part of other components, such as `StreamCallContent`, or as standalone widgets in your application, as long as the `call` and `participant` are supplied.