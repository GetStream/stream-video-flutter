...
You can see that as a first step, we need to create a call using the `makeCall()` method based on `type` and `id`, after which we can create/get the call using the `call.getOrCreate()` method.
