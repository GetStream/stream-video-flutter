...
When the iOS app is terminated the Flutter engine is not running and we need to start it up to handle the Stream call events. This is done internally by the SDK, the only thing needed (similarly to Android) is to register a callback handle that will run when Flutter engine starts and connect to StreamVideo.
